...
def load():
    print("Loading...")
    data = pd.read_csv("probT/datasets/phpBXvURX.csv")
    arff_data = arff.load(open("probT/datasets/phpBXvURX.arff"))


    for i in arff_data["attributes"]:
        if 'Class' in i[0] or 'class' in i[0]:
            np.save("probT/datasets/original/Class", np.asarray(i[1]))
    ...
    np.save("probT/datasets/original/train_X", train_X)
    np.save("probT/datasets/original/train_y", train_y)
    np.save("probT/datasets/original/valid_X", valid_X)
    np.save("probT/datasets/original/valid_y", valid_y)
    np.save("probT/datasets/original/test_X", test_X)
    np.save("probT/datasets/original/test_y", test_y)

    features_names = np.delete(data.columns, [3])
    np.save("probT/datasets/original/names", features_names)




    np.save("probT/datasets/custom/train_X", train_X)
    np.save("probT/datasets/custom/train_y", train_y)
    np.save("probT/datasets/custom/valid_X", valid_X)
    np.save("probT/datasets/custom/valid_y", valid_y)
    np.save("probT/datasets/custom/test_X", test_X)
    np.save("probT/datasets/custom/test_y", test_y)

    np.save("probT/datasets/custom/names", features_names)
