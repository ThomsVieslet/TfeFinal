...
def load():
    print("Loading...")
    data = pd.read_csv("probD/datasets/phpZvTVNJ.csv")
    arff_data = arff.load(open("probD/datasets/phpZvTVNJ.arff"))


    for i in arff_data["attributes"]:
        if i[0] == 'Class':
            np.save("probD/datasets/original/Class", np.asarray(i[1]))
    ...
    np.save("probD/datasets/original/train_X", train_X)
    np.save("probD/datasets/original/train_y", train_y)
    np.save("probD/datasets/original/valid_X", valid_X)
    np.save("probD/datasets/original/valid_y", valid_y)
    np.save("probD/datasets/original/test_X", test_X)
    np.save("probD/datasets/original/test_y", test_y)

    features_names = np.delete(data.columns, [3])
    np.save("probD/datasets/original/names", features_names)




    np.save("probD/datasets/custom/train_X", train_X)
    np.save("probD/datasets/custom/train_y", train_y)
    np.save("probD/datasets/custom/valid_X", valid_X)
    np.save("probD/datasets/custom/valid_y", valid_y)
    np.save("probD/datasets/custom/test_X", test_X)
    np.save("probD/datasets/custom/test_y", test_y)

    np.save("probD/datasets/custom/names", features_names)
