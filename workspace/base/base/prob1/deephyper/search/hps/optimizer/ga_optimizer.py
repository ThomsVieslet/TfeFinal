...
class GAOptimizer:
    ...
    def __init__(self, problem, num_workers, seed=SEED, CXPB=0.5, MUTPB=0.2, **kwargs):
        self.SEED = seed
        self.CXPB = CXPB
        self.MUTPB = MUTPB
        self.NGEN = kwargs.get("ga_num_gen")

        pop_size = num_workers * kwargs.get("individuals_per_worker")
        self.INIT_POP_SIZE = pop_size

        jLower = []
        jUpper = []

        for i in problem.space.get_hyperparameters():
            if hasattr(i, 'lower'):
                jLower.append(i.lower)
                jUpper.append(i.upper)
...
class SpaceEncoder:
    ...
    def encode_space(self):
        for p in self.space.get_hyperparameters():
    ...
    def encode(self, val):
        ttype = 'i'
        if type(val) == CS.hyperparameters.CategoricalHyperparameter:
            encoder = LabelEncoder()
            encoder.fit(val.choices)
            ttype = 'c'
        else:
            if hasattr(val, 'lower'):
                encoder = MinMaxScaler()
                encoder.fit(np.asarray(val).reshape(-1, 1))
                if itype(val) != (CS.hyperparameters.UniformIntegerHyperparameter or CS.hyperparameters.NormalIntegerHyperparameter) :
                    ttype = 'f'
