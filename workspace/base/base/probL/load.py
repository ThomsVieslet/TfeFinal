...
def load():
    print("Loading...")
    data = pd.read_csv("probL/datasets/visualizing_soil.csv")
    arff_data = arff.load(open("probL/datasets/visualizing_soil.arff"))


    for i in arff_data["attributes"]:
        if 'Class' in i[0] or 'class' in i[0]:
            np.save("probL/datasets/original/Class", np.asarray(i[1]))
    ...
    np.save("probL/datasets/original/train_X", train_X)
    np.save("probL/datasets/original/train_y", train_y)
    np.save("probL/datasets/original/valid_X", valid_X)
    np.save("probL/datasets/original/valid_y", valid_y)
    np.save("probL/datasets/original/test_X", test_X)
    np.save("probL/datasets/original/test_y", test_y)

    features_names = np.delete(data.columns, [4])
    np.save("probL/datasets/original/names", features_names)




    np.save("probL/datasets/custom/train_X", train_X)
    np.save("probL/datasets/custom/train_y", train_y)
    np.save("probL/datasets/custom/valid_X", valid_X)
    np.save("probL/datasets/custom/valid_y", valid_y)
    np.save("probL/datasets/custom/test_X", test_X)
    np.save("probL/datasets/custom/test_y", test_y)

    np.save("probL/datasets/custom/names", features_names)
